...
<?=ListView::widget([
    ...
    'itemView' => '_item',
    'layout' => "<p class='text-right'>{summary}</p><div class='row articles-items'>{items}</div><nav aria-label=\"Page navigation\">{pager}</nav>",
    'summary' => "Вы просматриваете с {begin} по {end} статью из {totalCount}",
